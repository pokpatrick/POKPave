...
let namesOfValue a =
  SSet.union (freeNamesOfValue a) (boundNamesOfValue a);;
...
let names p =
  SSet.union (freeNames p) (boundNames p);;
...
let fresh p forbid =
  let ns = names p in 
  let rec try_fresh n =
    let fname = ("f_" ^ (string_of_int n)) in 
      if (SSet.mem fname ns) || (SSet.mem fname forbid) then
	try_fresh (n+1)
      else
	fname
  in
...
let rec subst p m (* overrides *) n = 
  match p with
	...
	if a = n then
	  ...
	  if a = m then
	    let fname = fresh q SSet.empty
	    in
	      Res(fname, (subst (subst q fname a) m n))
	  else
	    Res(a, (subst q m n))
