...
let string_of_list tostr lst = 
  string_of_collection "[" "]" ";" tostr lst;;

let string_of_args tostr lst = 
  string_of_collection "(" ")" "," tostr lst;;
...
let rec permutations =
  let rec inject_all e n l llen = 
    if n > llen then
      ...
      (inject e n l)::(inject_all e (n+1) l llen)
  and inject e n l =
    if n = 0 then
      e::l
    else
      (List.hd l)::(inject e (n-1) (List.tl l))
  in
    function
      | [] -> []
      | e::[] -> [[e]]
...
let list_take l n =
  let rec take head tail n =
    if n = 0 then
      (List.hd tail, (List.rev head) @ (List.tl tail))
    else
      take ((List.hd tail)::head) (List.tl tail) (n - 1)
  in
    take [] l n;;
...
let list_inject e n l =
  let rec inject n head tail =
    if n = 0 then
      (List.rev head) @ (e::tail)
    else
      inject (n - 1) (List.hd tail::head) (List.tl tail)
  in 
    inject n [] l;;
...
let rec list_replace e n l =
  if n = 0 then
    ...
    (List.hd l)::(list_replace e (n-1) (List.tl l));;

(*Lee-Pok*)
let rec set_of_list list =
  match list with
    | [] -> SSet.empty
    | head::tail -> SSet.add head (set_of_list tail);;
