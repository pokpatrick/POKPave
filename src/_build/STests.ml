...
let pileGagne =
  Sum
    (Prefix (In("pile"), Prefix(Out("gagne"), Silent)),
...
let def_com =
  Res("a", Par(Prefix(Out("a"), Silent),
	       Prefix(In("a"), Silent)));;

let def_com' = 
  Res("b", Par(Prefix(Out("b"), Silent),
	       Prefix(In("b"), Silent)));;

let def_res =
  Par
    ((Res
	("a", (Par ((Prefix(Out("a"), Silent)),
...
Hashtbl.add (Control.global_definition_map) "def_PileGagne" (Definition("PileGagne", [], pileGagne));;
Hashtbl.add (Control.global_definition_map) "def_Once" (Definition("OneShot", [], once));;

let set = Semop.derivatives Control.global_definition_map (normalize piece);;
