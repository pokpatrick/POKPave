...
let rand_proc n =
  let rec f i vars =
	...
	match Random.int (if i = 1 then 2 else 4) with
	  ...
	  | 2 ->
	      let left_i = Random.int (pred i)
	      in
	      let right_i = i - left_i - 1
	      in
...
let rec perm_saps proc =
  ...
  let tree_of_sop cons sop =
	...
	if delta = 1 then
	  ...
	  let pivo = min + 1 + Random.int (delta - 1)
	  in
...
let alea_nus proc =
  let rec f map proc =
    match proc with
	  ...
	  in
	  let poss = SSet.diff defined frees
	  in
	  let new_name =
	    let nb = SSet.cardinal poss
	    in
	      if nb > 0 then
		List.nth (SSet.elements poss) (Random.int nb)
	      else
		let rec create n =
		  let s = "f" ^ (string_of_int n)
		  in
		    if SSet.mem s poss then
		      create (succ n)
		    else
		      s
		in
...
let normalize_random_check proc_sizes repeat_nb =
  ...
  for i = 1 to repeat_nb do
    let proc = rand_proc proc_sizes
    in
    let nproc = normalize proc
    in
    let rand_proc = perm_proc proc
    in
    let rand_nproc = normalize proc
    in
...
let normalize_exhaustive_check child_sizes child_nb =
  let childs = Array.init child_nb (fun _ -> rand_proc child_sizes)
  in
  let main_proc =
    let rec f n =
      if n = 0 then
	...
	Sum (childs.(n), f (pred n))
    in
      f (pred child_nb);
  in
  let main_normalized = normalize main_proc
  in
  let check_proc proc =
    let nproc = normalize proc
    in
      if nproc <> main_normalized
      then (
	eprintf "\nNormalization error:\
\
  normalize( %s )\
