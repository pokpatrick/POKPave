...
let build_graph init_graph init_partition defs ps =
  ...
  let rec f (graph, part) procs_todo procs_done =
	...
	let folder ((src, _, dst) as trans) (accg, accp) =
	  let (src', lbl', dst') =
	    (PState(false, src), LState trans, PState(false, dst))
	  in
	  let accg' = GMap.add lbl' (GSet.singleton src') accg
	  in
	  let accp' = add_to_partition accp lbl'
	  in
	    if GMap.mem dst' accg' then
	      let l = GMap.find dst' accg'
	      in
		(GMap.add dst' (GSet.add lbl' l) accg', accp')
	    else
	      (GMap.add dst' (GSet.singleton lbl') accg', add_to_partition accp' dst')
	in
  ...
  in
    match ps with
	  ...
	  in
	    f tmp (PSet.singleton p2) PSet.empty
...
let rec refine (graph, part) =
  ...
  in
  let split part prev = 
    let p1 = GSet.inter part prev
    in
    let p2 = GSet.diff part prev
    in
      (p1, p2)
  in
  let rec f1 pt pr =
    ...
    match pr with
	  ...
	  in
	    if GSet.is_empty spl1 or GSet.is_empty spl2 then
	      ...
	      [spl1; spl2]
  in
  let part' = f1 part prevs
  in
    if (List.length part = List.length part') then
      part'
    else
      refine (graph, part')
...
let minimize defs proc =
  let init_graph = GMap.add (PState(false,proc)) GSet.empty GMap.empty
  in
  let init_partition = [GSet.singleton (PState(false,proc))]
  in
  let (graph, partition) = build_graph init_graph init_partition defs [proc]
  in
  ...
  in
    (*print_string "PARTITION REFINED :";
      print_string (string_of_partition partition');*)
  let transitions = build_lts partition'
  in
    TsSet.fold (fun t acc -> t :: acc) transitions []
      
let is_fbisimilar defs p1 p2 =
  let root = PState(true,(SSet.empty, NSilent))
  in
  let pst1 = PState(false, p1)
  in
  let pst2 = PState(false, p2)
  in
  let init_graph =
    GMap.add pst2 (GSet.singleton root)
      (GMap.add pst1 (GSet.singleton root)
	 (GMap.add root GSet.empty GMap.empty))
  in
  let init_partition =
    [GSet.add pst1 (GSet.add pst2 (GSet.singleton root))]
  in
  let (graph, partition) =
    build_graph init_graph init_partition defs [p1; p2]
  in
  let partition' = refine (graph, partition)
  in
