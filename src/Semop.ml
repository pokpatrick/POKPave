...
let derivatives defs ((orig_res, orig_np) as orig_nproc) =
  ...
  let rec f res np =
    match np with
      | NSilent -> TSet.empty
      | NCall (name, args) ->
	  let def_sign = string_of_def_header (Definition(name, args, Silent))
	  in
	  let Definition (_, _, body) =
	    try
	      Hashtbl.find defs def_sign
	    with Not_found -> failwith (sprintf "Process %s not found" name)
	  in
	  let (body_res, body_np) = normalize body
	  in
	  let derivs = f body_res body_np
	  in
	  ...
	  List.fold_left (fun acc np -> TSet.union (f res np) acc)
	    TSet.empty nps
      | NPar nps ->
	  let np_frees = SSet.diff (nfreeNames np) res
	  in
	  let ((set_par : TSet.t), (set_simpl : (TSet.t * nproc list) list)) =
	    let folder (acc_par, acc_simpl) np =
	      let oths_np = List.filter ((!=) np) nps
	      in
	      let nexts = f SSet.empty np
	      in
	      let folder (src, lab, (in_res, dst)) acc =
		let new_dst =
		  let in_frees = SSet.diff (nfreeNames dst) in_res
		  in
		  let in_forbid = SSet.inter in_res np_frees
		  in
		  let np_forbid = SSet.inter res in_frees
		  in
		  let folder_in name (cnt, acc_in, acc_in_res) =
		    let rec gen_name n =
		      let new_n = "f" ^ (string_of_int n)
		      in
			if SSet.mem new_n in_frees || SSet.mem new_n in_res then
			  gen_name (succ n)
			else
			  (succ n, new_n)
		    in
		    let (new_cnt, new_name) = gen_name cnt
		    in
		  ...
		  let folder_oths name (cnt, acc_oths, acc_oths_res) =
		    let rec gen_name n =
		      let new_n = "f" ^ (string_of_int n)
		      in
			if SSet.mem new_n np_frees || SSet.mem new_n res then
			  gen_name (succ n)
			else
			  (succ n, new_n)
		    in
	  ...
	  let folder ((map, _) as acc) (elt_set, oths) =
		...
		let new_map' =
		  try
		    SMap.add org ((dst,oths) :: SMap.find org map') map'
		  with Not_found -> SMap.add org [(dst,oths)] map'
		in
		  try
		    let dsts' = SMap.find opp map
		    in
			...
			List.filter (fun e -> List.memq e oths) oths'
		      in
		      let nproc' =
			let p1 = denormalize np
			in
			let p2 = denormalize dst
			in
			let oths_p =
...
let lts defs p =
  let rec f acc_trans procs_todo procs_done =
    try
      let p = PSet.choose procs_todo
      in
      let derivs = derivatives defs p
      in
      let new_acc_trans = TSet.union acc_trans derivs
      in
      let next_procs =
	...
	PSet.remove p (PSet.union (PSet.diff next_procs procs_done) procs_todo)
      in
      let new_procs_done = PSet.add p procs_done
      in
	f new_acc_trans new_procs_todo new_procs_done
    with Not_found -> acc_trans
  in
  let transs = f TSet.empty (PSet.singleton p) PSet.empty
  in
    TSet.fold (fun t acc -> t :: acc) transs [];;

let construct_bisimilarity defs nproc1 nproc2 =
  let rec construct bsm np1 np2 =
    let d1s = derivatives defs np1
    in
    let d2s = derivatives defs np2
    in
    let folder dys inv (_, labx, dstx) acc_bsm =
      let rec search acc_dys =
	if TSet.is_empty acc_dys then
	  failwith "Bad path"
	else
	  let ((_, laby, dsty) as ty) = TSet.choose acc_dys
	  in
	    if labx <> laby then
	      search (TSet.remove ty acc_dys)
	    else
	      let dst1 =
		if inv then
		  dsty
		...
		if BSet.mem dsts acc_bsm || BSet.mem (dst2, dst1) acc_bsm || dst1 = dst2 then
		  acc_bsm
		else
		  try
		    construct (BSet.add dsts acc_bsm) dst1 dst2
		  with Failure "Bad path" -> search (TSet.remove ty acc_dys)
      in
	search dys
    in
      TSet.fold (folder d2s false) d1s (TSet.fold (folder d1s true) d2s bsm)
  in
    try
      construct (BSet.singleton (nproc1, nproc2)) nproc1 nproc2
    with Failure "Bad path" -> failwith "Not bisimilar";;

let is_bisimilar defs nproc1 nproc2 =
  try
    ignore (construct_bisimilarity defs nproc1 nproc2);
    true
